...
"""Numeric-only clustering: numerical Gower distance + Ng-Jordan-Weiss.

Place this file in::

    Molecular-Bioresponse/models/numerico/NJW_numeric_only_retest.py

The script performs a strict block-ablation of the mixed-data analysis:

    retained numerical descriptors
        -> feature-wise min-max normalization
        -> numerical Gower dissimilarity
...
SCRIPT_VERSION = "numeric-only-retest-v1-2026-07-22"

# Change only this name when creating another parallel experiment.
# The baseline folder "njw_numeric_only" is intentionally protected.
RUN_NAME = "njw_numeric_only_retest"
RANDOM_STATE = 42
TARGET_COL = "Activity"
...
if RUN_NAME == "njw_numeric_only":
    raise ValueError(
        "RUN_NAME cannot be 'njw_numeric_only': that folder contains the valid baseline outputs."
    )

REPORT_DIR = SCRIPT_DIR / "reports" / RUN_NAME
...
RANKING_CSV = TABLE_DIR / "03_numeric_candidate_ranking.csv"
RESOLUTION_SUMMARY_CSV = TABLE_DIR / "04_numeric_resolution_summary.csv"
ARI_COMPARISON_CSV = TABLE_DIR / "05_numeric_cross_representation_ari.csv"
NUMERIC_K2_K3_COUNTS_CSV = TABLE_DIR / "06_numeric_k2_vs_k3_counts.csv"
NUMERIC_K2_K3_ROWS_CSV = TABLE_DIR / "07_numeric_k2_vs_k3_row_percentages.csv"
NUMERIC_K2_K4_COUNTS_CSV = TABLE_DIR / "08_numeric_k2_vs_k4_counts.csv"
NUMERIC_K2_K4_ROWS_CSV = TABLE_DIR / "09_numeric_k2_vs_k4_row_percentages.csv"
...
def compute_numeric_gower_dissimilarity(
    ...
    """Compute the numerical component of Gower dissimilarity.

    After feature-wise min-max scaling, this is the mean L1 distance:

        d(i,l) = (1 / p_num) * sum_j |x_ij - x_lj|.

    The chunked implementation limits temporary memory while preserving the
    exact full pairwise matrix required by the downstream NJW analysis.
    """
...
def run_complete_grid(distance: np.ndarray) -> pd.DataFrame:
    ...
    sigma_reference = median_positive_distance(distance)
    print(f"Median positive numerical Gower dissimilarity: {sigma_reference:.6f}")
    ...
    for sigma_index, multiplier in enumerate(SIGMA_MULTIPLIERS):
        ...
        for k in K_VALUES:
            ...
            print(
                f"k={k:>2} | "
                f"sil_numG={solution.numeric_gower_silhouette: .4f} | "
...
def rank_candidates(results: pd.DataFrame) -> pd.DataFrame:
    """Rank configurations only inside the numerical-Gower geometry."""
...
def save_raw_vs_normalized(solution: FittedSolution, output_name: str) -> None:
    ...
    fig.suptitle(
        f"Numeric-only Gower + NJW | k={solution.k} | "
...
def write_automatic_interpretation(
    ...
    lines = [
        ...
        (
            ...
            f"numerical-Gower silhouette was {selected.numeric_gower_silhouette:.4f}, "
    ...
    lines.extend([
        ...
        (
            ...
            "comparisons. Numerical-Gower, asymmetric-Jaccard, and mixed-Gower "
...
def main() -> None:
    ...
    save_nesting_heatmap(
        k2_k3_counts,
        k2_k3_rows,
        title="Numeric-only nesting: k=2 versus k=3",
        output_name="09_numeric_k2_vs_k3_nesting.png",
    )
    save_nesting_heatmap(
        k2_k4_counts,
        k2_k4_rows,
        title="Numeric-only nesting: k=2 versus k=4",
        output_name="10_numeric_k2_vs_k4_nesting.png",
    )
    save_ari_comparison_figure(
        comparisons,
        output_name="11_numeric_cross_representation_ari.png",
    )
    ...
    report_values = {
        ...
        "important_interpretation_rule": (
            ...
            "numerical-Gower, asymmetric-Jaccard, and mixed-Gower silhouettes directly."
        ),
    }
