...
2. Repeated screening of several (k_rows, k_columns) configurations.
...
6. Conditional column-wise permutation diagnostic preserving each feature's
...
The permutation diagnostic is conditional on the selected configuration. It is
not a selection-adjusted confirmatory test, because the complete model-selection
pipeline is not repeated for every permuted matrix.
...
class Config:
    ...
    input_path: Path = PROJECT_DIR / "000_Dataset" / "train_filtered_no_activity.csv"
    target_path: Path = PROJECT_DIR / "000_Dataset" / "train_activity_target.csv"
    report_dir: Path = (
        PROJECT_DIR
        / "003_Models"
        / "0031_Mixed"
    ...
    methods: tuple[str, ...] = ("scale",)
    row_cluster_values: tuple[int, ...] = (2, 3, 4, 5, 6)
    column_cluster_values: tuple[int, ...] = (3, 4, 5, 6, 7, 8)
    screening_seeds: tuple[int, ...] = (42, 7, 99)

    # SpectralBiclustering parameters.
    n_components: int = 10
    n_best: int = 5
    n_init: int = 20
    svd_method: str = "randomized"

    # Profiling, candidate selection and stability.
    ...
    minimum_screening_success_fraction: float = 1.0
    stability_seeds: tuple[int, ...] = (42, 7, 99, 123, 2026)
    minimum_stability_success_fraction: float = 0.8
    minimum_row_ari: float = 0.8
    minimum_column_ari: float = 0.8
    reject_numerical_warnings: bool = False
...
# ---------------------------------------------------------------------------
# General utilities and data loading
# ---------------------------------------------------------------------------
...
def parse_arguments() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        default=None,
        help="Override the configured normalization method.",
    )
    parser.add_argument(
        "--quick",
        action="store_true",
...
def validate_config(config: Config) -> None:
    """Fail early on ambiguous or internally inconsistent settings."""
    if isinstance(config.methods, str) or not isinstance(config.methods, tuple):
        raise TypeError(
            "Config.methods must be a tuple, for example ('scale',), not a string."
        )
    valid_methods = {"bistochastic", "scale", "log"}
    unknown_methods = set(config.methods) - valid_methods
    if not config.methods or unknown_methods:
        raise ValueError(
            f"Config.methods must contain only {sorted(valid_methods)}; "
            f"received {config.methods!r}."
        )
    integer_sequences = {
        "row_cluster_values": config.row_cluster_values,
        "column_cluster_values": config.column_cluster_values,
        "screening_seeds": config.screening_seeds,
        "stability_seeds": config.stability_seeds,
    }
    for name, values in integer_sequences.items():
        if isinstance(values, (str, bytes)) or not values:
            raise ValueError(f"{name} must be a non-empty sequence.")
        if len(set(values)) != len(values):
            raise ValueError(f"{name} contains duplicate values: {values!r}.")
    if min(config.row_cluster_values) < 2:
        raise ValueError("Every row-cluster value must be at least 2.")
    if min(config.column_cluster_values) < 2:
        raise ValueError("Every column-cluster value must be at least 2.")
    if config.n_components < 1 or not 1 <= config.n_best <= config.n_components:
        raise ValueError("Require n_components >= 1 and 1 <= n_best <= n_components.")
    if config.n_init < 1:
        raise ValueError("n_init must be positive.")
    if config.number_of_candidates < 1:
        raise ValueError("number_of_candidates must be positive.")
    for name, value in (
        ("minimum_screening_success_fraction", config.minimum_screening_success_fraction),
        ("minimum_stability_success_fraction", config.minimum_stability_success_fraction),
        ("minimum_row_ari", config.minimum_row_ari),
        ("minimum_column_ari", config.minimum_column_ari),
        ("minimum_row_entropy", config.minimum_row_entropy),
        ("minimum_column_entropy", config.minimum_column_entropy),
    ):
        if not 0.0 <= value <= 1.0:
            raise ValueError(f"{name} must lie in [0, 1].")
    if config.r2_tolerance < 0:
        raise ValueError("r2_tolerance must be non-negative.")
    if config.null_permutations < 0:
        raise ValueError("null_permutations must be non-negative.")
...
def load_dataset(config: Config, output_paths: dict[str, Path]) -> DatasetBundle:
    ...
    if values.min() < 0:
        raise ValueError("SpectralBiclustering requires a non-negative matrix.")
    ...
    if not np.all(valid_rows):
        pd.DataFrame({"removed_zero_sum_original_row": original_row_ids[~valid_rows]}).to_csv(
            output_paths["root"] / "removed_zero_sum_rows.csv", index=False
        )
    X = X.loc[valid_rows].reset_index(drop=True)
    original_row_ids = original_row_ids[valid_rows]
    if max(config.row_cluster_values) > len(X):
        raise ValueError("The requested number of row clusters exceeds the row count.")
    if max(config.column_cluster_values) > X.shape[1]:
        raise ValueError("The requested number of column clusters exceeds the feature count.")
    if config.n_components > min(X.shape):
        raise ValueError(
            "n_components cannot exceed the smaller matrix dimension after zero-sum removal."
        )
    ...
    if config.target_path.exists():
        ...
        logging.warning(
            "Target file not found. Activity-based external interpretation will be skipped: %s",
            config.target_path,
        )
    ...
    return DatasetBundle(
        X=X, y=y, original_row_ids=original_row_ids, feature_profiles=feature_profiles
    )
...
def normalized_entropy(cluster_sizes: np.ndarray) -> float:
    """Entropy normalized by the requested number of clusters.

    Zero-sized clusters remain in the denominator, so a partition that uses
    fewer clusters than requested cannot appear perfectly balanced.
    """
    sizes = np.asarray(cluster_sizes, dtype=float)
    if sizes.ndim != 1 or len(sizes) <= 1:
        return 0.0
    if np.any(sizes < 0) or sizes.sum() <= 0:
        raise ValueError("Cluster sizes must be non-negative and have a positive sum.")
    probabilities = sizes[sizes > 0] / sizes.sum()
    entropy = -np.sum(probabilities * np.log(probabilities))
    return float(entropy / np.log(len(sizes)))
...
def compute_block_statistics(
            ...
            records.append(
                {
                    **metadata,
                    ...
                    "column_cluster_global_zero_fraction": column_cluster_zero_fraction,
                    "zero_fraction_contrast_to_column_cluster": block_zero_fraction
    ...
    aggregate = {
        ...
        "block_zero_fraction_range": float(
            block_frame["block_zero_fraction"].max() - block_frame["block_zero_fraction"].min()
        ),
    }
...
def compute_column_cluster_statistics(
    column_labels: np.ndarray, feature_profiles: pd.DataFrame, metadata: dict[str, Any]
) -> pd.DataFrame:
        ...
        records.append(
            {
                **metadata,
                ...
                "feature_cluster_fraction": safe_fraction(n_features, len(assignments)),
                "n_binary_features": n_binary,
                "n_numeric_features": int(n_features - n_binary),
                "binary_feature_fraction": safe_fraction(n_binary, n_features),
                "numeric_feature_fraction": safe_fraction(n_features - n_binary, n_features),
                "n_sparse_features": n_sparse,
                "sparse_feature_fraction": safe_fraction(n_sparse, n_features),
                "mean_feature_zero_fraction": float(group["zero_fraction"].mean()),
                "median_feature_zero_fraction": float(group["zero_fraction"].median()),
                "minimum_feature_zero_fraction": float(group["zero_fraction"].min()),
                "maximum_feature_zero_fraction": float(group["zero_fraction"].max()),
...
def compute_activity_association(
    row_labels: np.ndarray, y: pd.Series | None
    ...
    cramers_v = (
        float(np.sqrt(chi2 / (n * min_dimension))) if n > 0 and min_dimension > 0 else float("nan")
    )
...
def compute_partition_metrics(
    ...
    blocks, block_metrics = compute_block_statistics(
        values, row_labels, column_labels, feature_profiles, metadata
    )
    ...
    metrics: dict[str, Any] = {
        ...
        **block_metrics,
        ...
        "smallest_row_cluster_fraction": safe_fraction(row_sizes.min(), row_sizes.sum()),
        "largest_row_cluster_fraction": safe_fraction(row_sizes.max(), row_sizes.sum()),
        ...
        "smallest_column_cluster_fraction": safe_fraction(column_sizes.min(), column_sizes.sum()),
        "largest_column_cluster_fraction": safe_fraction(column_sizes.max(), column_sizes.sum()),
        "feature_type_nmi": float(normalized_mutual_info_score(binary_flag, column_labels)),
        "sparse_flag_nmi": float(normalized_mutual_info_score(sparse_flag, column_labels)),
        "feature_zero_fraction_eta2": eta_squared(zero_fraction, column_labels),
        **activity_metrics,
    }
    return (metrics, blocks, columns)


def validate_partition_labels(
    labels: np.ndarray,
    expected_length: int,
    ...
    if labels.shape != (expected_length,):
        raise RuntimeError(
            f"Invalid {label_kind}-label shape: {labels.shape}; "
            f"expected {(expected_length,)}."
        )
    observed = np.unique(labels)
    expected = np.arange(expected_clusters, dtype=int)
    if not np.array_equal(observed, expected):
        raise RuntimeError(
            f"Degenerate {label_kind} partition: requested {expected_clusters} clusters "
            f"but obtained labels {observed.tolist()}."
        )
    return labels.copy()
...
def fit_spectral_labels(
    ...
    """Fit once and reject incomplete or numerically unreliable partitions."""
    ...
    critical = [
        ...
        if issubclass(record.category, (RuntimeWarning, ConvergenceWarning))
    ]
    if config.reject_numerical_warnings and critical:
        raise RuntimeError(
            "Numerical/convergence warning during SpectralBiclustering.fit: "
            + " | ".join(dict.fromkeys(critical))
        )
    for message in warning_messages:
        logging.warning(
            "Non-fatal fit warning for %s r=%d c=%d seed=%d: %s",
            method,
            n_row_clusters,
            n_column_clusters,
            seed,
    ...
    row_labels = validate_partition_labels(
        model.row_labels_, values.shape[0], n_row_clusters, "row"
    )
    column_labels = validate_partition_labels(
        model.column_labels_, values.shape[1], n_column_clusters, "column"
    )
    return row_labels, column_labels, warning_messages
...
def fit_partition(
    ...
    row_labels, column_labels, fit_warnings = fit_spectral_labels(
        bundle.values,
        config,
        method,
        n_row_clusters,
    ...
    metrics, blocks, columns = compute_partition_metrics(
        bundle.values, row_labels, column_labels, bundle.feature_profiles, bundle.y, metadata
    )
    required_metrics = (
        "block_r2",
        "weighted_h_score",
    ...
    non_finite = [
        name for name in required_metrics if not np.isfinite(float(metrics[name]))
    ]
    if non_finite:
        raise RuntimeError(f"Non-finite partition metrics: {non_finite}.")
    metrics["fit_warning_count"] = len(fit_warnings)
    metrics["fit_warnings"] = " | ".join(fit_warnings)
...
def run_screening(
    bundle: DatasetBundle, config: Config, output_paths: dict[str, Path]
) -> pd.DataFrame:
    configurations = list(
        product(
            config.methods,
    ...
    if all_blocks:
        pd.concat(all_blocks, ignore_index=True).to_csv(
            output_paths["screening"] / "screening_block_statistics.csv", index=False
        )
    if all_column_stats:
        pd.concat(all_column_stats, ignore_index=True).to_csv(
            output_paths["screening"] / "screening_column_cluster_statistics.csv", index=False
        )
    if all_row_assignments:
        pd.concat(all_row_assignments, ignore_index=True).to_csv(
            output_paths["screening"] / "screening_row_assignments.csv", index=False
        )
    if all_column_assignments:
        pd.concat(all_column_assignments, ignore_index=True).to_csv(
            output_paths["screening"] / "screening_column_assignments.csv", index=False
        )
    successful = runs[runs["status"] == "ok"].copy()
    if successful.empty:
        raise RuntimeError("All screening configurations failed. Inspect screening_runs.csv.")
    grouping_keys = ["method", "n_row_clusters", "n_column_clusters"]
    coverage = (
        runs.groupby(grouping_keys, as_index=False)
        .agg(
            screening_runs_requested=("status", "size"),
            screening_runs_successful=("status", lambda status: int((status == "ok").sum())),
        )
    )
    coverage["screening_success_fraction"] = (
        coverage["screening_runs_successful"] / coverage["screening_runs_requested"]
    )
    coverage["meets_screening_success_requirement"] = (
        coverage["screening_success_fraction"]
        >= config.minimum_screening_success_fraction
    )
    metric_columns = [
        "block_r2",
        "weighted_h_score",
    ...
    comparison = successful.groupby(
        ["method", "n_row_clusters", "n_column_clusters", "n_blocks"], as_index=False
    ...
    comparison = comparison.merge(coverage, on=grouping_keys, how="left")
...
def select_candidates(
    comparison: pd.DataFrame, config: Config, output_paths: dict[str, Path]
) -> pd.DataFrame:
    reliable = comparison[
        comparison["meets_screening_success_requirement"].fillna(False)
    ].copy()
    if reliable.empty:
        raise RuntimeError(
            "No configuration met the screening-success requirement. "
            "Inspect screening_runs.csv before changing any threshold."
        )
    if config.manual_candidates:
        records: list[pd.DataFrame] = []
        for rank, (method, n_rows, n_columns) in enumerate(config.manual_candidates, start=1):
            match = reliable[
                (reliable["method"] == method)
                & (reliable["n_row_clusters"] == n_rows)
                & (reliable["n_column_clusters"] == n_columns)
            ].copy()
            if match.empty:
                raise ValueError(
                    "Manual candidate absent or unreliable in the screening grid: "
        ...
        for method, method_data in reliable.groupby("method", sort=False):
            best_r2 = float(method_data["block_r2_mean"].max())
            eligible = method_data[
                (method_data["block_r2_mean"] >= best_r2 - config.r2_tolerance)
                & (method_data["row_cluster_entropy_mean"] >= config.minimum_row_entropy)
                & (method_data["column_cluster_entropy_mean"] >= config.minimum_column_entropy)
            ].copy()
            if eligible.empty:
                failed_methods.append(str(method))
                continue
        ...
        if failed_methods:
            logging.warning(
                "No configuration satisfied all quality constraints for methods: %s",
                ", ".join(failed_methods),
            )
        if not eligible_parts:
            raise RuntimeError(
                "No screened configuration satisfies the R²-tolerance and entropy "
                "constraints. The pipeline will not silently select a degenerate fallback."
            )
        pool = pd.concat(eligible_parts, ignore_index=True)
        pool = pool.sort_values(
            ["n_blocks", "block_r2_mean", "weighted_h_score_mean", "row_cluster_entropy_mean"],
        ...
        candidates["selection_reason"] = (
            f"R² within {config.r2_tolerance:.3f} of method maximum; then minimum number of blocks"
        )
        ...
        records.append(
            {
                ...
                "ari": float(adjusted_rand_score(label_sets[first_seed], label_sets[second_seed])),
            }
        )
    return records
...
def seed_centrality(
    row_labels: dict[int, np.ndarray], column_labels: dict[int, np.ndarray]
) -> pd.DataFrame:
    ...
    for seed in seeds:
        ...
        if not other_seeds:
            ...
            row_scores = [
                adjusted_rand_score(row_labels[seed], row_labels[other]) for other in other_seeds
            ]
            column_scores = [
                adjusted_rand_score(column_labels[seed], column_labels[other])
...
# ---------------------------------------------------------------------------
# Stability analysis and final-model selection
# ---------------------------------------------------------------------------


def run_stability_analysis(
    candidates: pd.DataFrame, bundle: DatasetBundle, config: Config, output_paths: dict[str, Path]
    ...
    for candidate in candidates.itertuples(index=False):
        ...
        for seed in config.stability_seeds:
            ...
            try:
                ...
                logging.exception(
                    "Stability fit failed: %s r=%d c=%d seed=%d", method, n_rows, n_columns, seed
                )
                run_records.append(
                    {
                        **model_metadata(method, n_rows, n_columns, seed),
    ...
    if centrality_records:
        pd.concat(centrality_records, ignore_index=True).to_csv(
            output_paths["candidates"] / "representative_seed_scores.csv", index=False
        )
    ...
    grouping_keys = ["method", "n_row_clusters", "n_column_clusters"]
    coverage = (
        stability_runs.groupby(grouping_keys, as_index=False)
        .agg(
            stability_runs_requested=("status", "size"),
            stability_runs_successful=("status", lambda status: int((status == "ok").sum())),
        )
    )
    coverage["stability_success_fraction"] = (
        coverage["stability_runs_successful"] / coverage["stability_runs_requested"]
    )
    coverage["enough_successful_stability_fits"] = (
        (coverage["stability_runs_successful"] >= 2)
        & (
            coverage["stability_success_fraction"]
            >= config.minimum_stability_success_fraction
        )
    )
    metric_columns = [
        "block_r2",
    ...
    stability_summary = successful.groupby(
    ...
    if not pairwise.empty:
        ari_summary = (
            pairwise.groupby(
                ["method", "n_row_clusters", "n_column_clusters", "label_kind"], as_index=False
            )["ari"]
        ...
        stability_summary = stability_summary.merge(
            row_ari, on=grouping_keys, how="left"
        ).merge(column_ari, on=grouping_keys, how="left")
    else:
    ...
    ) & (stability_summary["column_ari_mean"] >= config.minimum_column_ari)
    stability_summary = stability_summary.merge(coverage, on=grouping_keys, how="outer")
    stability_summary["meets_stability_threshold"] = (
        stability_summary["enough_successful_stability_fits"].fillna(False)
        & stability_summary["meets_stability_threshold"].fillna(False)
    )
    stability_summary = candidates[
        ["method", "n_row_clusters", "n_column_clusters", "candidate_rank"]
    ].merge(stability_summary, on=grouping_keys, how="left")
...
def choose_final_configuration(
    ...
    eligible = stability_summary[
        stability_summary["enough_successful_stability_fits"].fillna(False)
        & np.isfinite(stability_summary["minimum_mean_ari"])
    ].copy()
    if eligible.empty:
        raise RuntimeError(
            "No candidate completed enough stability fits to support final selection."
        )
    stable = eligible[eligible["meets_stability_threshold"].fillna(False)].copy()
    if not stable.empty:
        chosen_row = stable.sort_values(
            ["candidate_rank", "minimum_mean_ari"], ascending=[True, False]
        ).iloc[0]
        selection_reason = "First parsimonious candidate satisfying both ARI thresholds"
    else:
        chosen_row = eligible.sort_values(
            ["minimum_mean_ari", "candidate_rank"], ascending=[False, True]
        ).iloc[0]
        selection_reason = (
            "Exploratory fallback: no candidate satisfied both ARI thresholds; "
            "selected the most stable sufficiently evaluated candidate"
        )
        logging.warning(selection_reason)
    key = (
    ...
    chosen = {
        ...
        "stability_success_fraction": float(chosen_row["stability_success_fraction"]),
        "stability_thresholds_met": bool(chosen_row["meets_stability_threshold"]),
    }
...
def representative_features(
    values: np.ndarray, column_labels: np.ndarray, feature_profiles: pd.DataFrame, top_n: int
) -> pd.DataFrame:
    """
            ...
            records.append(
                {
                    ...
                    "distance_to_cluster_centroid": float(distances[local_index]),
...
def save_final_outputs(
    ...
    row_assignments = pd.DataFrame(
        {"original_row_id": bundle.original_row_ids, "row_cluster": final_result.row_labels}
    )
    if bundle.y is not None:
    ...
    interesting_blocks = final_result.block_statistics.sort_values(
        ["absolute_mean_contrast", "block_h_score"], ascending=[False, True]
    ...
    final_summary = {
        ...
        **activity_metrics,
        "interpretation_warning": (
            "Feature clusters must not be interpreted biologically from sparsity "
            "or binary/numeric composition alone."
        ),
    }
...
def save_metric_grid(
    ...
    matrix = (
        method_data.pivot(index="n_row_clusters", columns="n_column_clusters", values=metric)
        .sort_index()
        .sort_index(axis=1)
    )
    if matrix.empty:
...
def save_screening_plots(comparison: pd.DataFrame, output_dir: Path, config: Config) -> None:
    metrics = [
        ...
        ("feature_zero_fraction_eta2_mean", "Feature sparsity explained (eta²)", ".3f"),
        ("largest_column_cluster_fraction_mean", "Largest column-cluster fraction", ".3f"),
    ]
...
def save_block_heatmap(
    ...
    matrix = (
        block_statistics.pivot(index="row_cluster", columns="column_cluster", values=metric)
        .sort_index()
        .sort_index(axis=1)
    )
...
def save_feature_composition_plot(
    column_summary: pd.DataFrame, output_path: Path, config: Config
) -> None:
...
def save_sparsity_boxplot(
    column_labels: np.ndarray, feature_profiles: pd.DataFrame, output_path: Path, config: Config
) -> None:
    groups = [
...
def save_final_plots(
    ...
    save_feature_composition_plot(
        final_result.column_cluster_statistics, final_dir / "feature_type_composition.png", config
    )
...
def write_interpretation_summary(
    ...
    lines = [
        ...
        (
            "- successful stability-fit fraction: "
            f"{final_selection['stability_success_fraction']:.3f}"
        ),
        (
            "- both prespecified ARI thresholds met: "
            f"{final_selection['stability_thresholds_met']}"
        ),
        "",
    ...
    lines.extend(
        [
            ...
            (
                "5. Use the conditional permutation diagnostic only as exploratory "
                "evidence about the fixed final configuration."
            ),
